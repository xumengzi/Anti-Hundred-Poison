...
function clearBaidu() {
  ...
  if (list || list1) {
    let keys = [
      ...
      '百度壁纸', '百度下载助手', '百度阅读器', '百度电脑专家', '百度一键root', '百度手柄', '百度汉语',
      '百度宝宝知道', '百度糯米', '百度图片', '百度阅读', '百度翻译', '百度魔拍', '百度刷机', '百度企业信用',
      '百度游戏', '百度外卖', '百度团购', '百度日历', '百度乐播', '百度相机', '百度贴吧青春版', '百度地图',
      '百度理财', '百度游戏大全', '百度多酷棋牌', '百度天眼', '百度财富', '百度医生', '百度网盘',
      '百度德州扑克', '百度路淘', '百度聚聚', '百度滴答', '百度钱包', '百度体育新闻', '百度搜索资源平台',
      '百度搜索风云榜', '百度智能云', '百度指数', '百度学术', '百度营销中心', '百度爱采购', '百度公益', '百度识图',
      '百度百聘', '百度号码认证平台', '百度商桥', '有钱花'
    ];
      ...
      newArr[i].forEach(item => {
        for (let i in keys) {
          if (item.innerText.includes(keys[i])) {
            ...
            item.style.boxShadow = '1px 1px 10px #ccc';
